...
void UResourceManager::InitShaderILMap()
{
    ...
    layout.Add({ "INSTANCE_WORLD0", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 1, 0,  D3D11_INPUT_PER_INSTANCE_DATA, 1 });
    layout.Add({ "INSTANCE_WORLD1", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 1, 16, D3D11_INPUT_PER_INSTANCE_DATA, 1 });
    layout.Add({ "INSTANCE_WORLD2", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 1, 32, D3D11_INPUT_PER_INSTANCE_DATA, 1 });
    layout.Add({ "INSTANCE_WORLD3", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 1, 48, D3D11_INPUT_PER_INSTANCE_DATA, 1 });

    layout.Add({ "INSTANCE_INVWORLD0", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 1, 64,  D3D11_INPUT_PER_INSTANCE_DATA, 1 });
    layout.Add({ "INSTANCE_INVWORLD1", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 1, 80, D3D11_INPUT_PER_INSTANCE_DATA, 1 });
    layout.Add({ "INSTANCE_INVWORLD2", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 1, 96, D3D11_INPUT_PER_INSTANCE_DATA, 1 });
    layout.Add({ "INSTANCE_INVWORLD3", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 1, 112, D3D11_INPUT_PER_INSTANCE_DATA, 1 });
    ...
    // ────────────────────────────────
    // Particle Beam 렌더링용 Shader
    // FParticleBeamVertex: Position(float3) + UV(float2) + Color(float4)
    // ────────────────────────────────
    layout.Add({ "POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT,    0, 0,
                 D3D11_INPUT_PER_VERTEX_DATA, 0 });
    layout.Add({ "TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT,       0, 12,
...
void UResourceManager::PreloadParticles()
{
    const fs::path ParticleDir(GDataDir + "/Particle");
    if (!fs::exists(ParticleDir) || !fs::is_directory(ParticleDir))
    {
        UE_LOG("UResourceManager::PreloadParticles: Data directory not found: %s", ParticleDir.string().c_str());
        return;
    }
    ...
    for (const auto& Entry : fs::recursive_directory_iterator(ParticleDir))
    {
        ...
        if (Extension == ".particle")
        {
            ...
            if (!ProcessedFiles.contains(PathStr))
            {
                ...
                // Load wav file 
                ++LoadedCount;
                Load<UParticleSystem>(Path.string());
            }
        }
    }
